...
test.describe('Responsive Navigation and Layout Tests', () => {
  ...
  test('Main navigation should be hidden on mobile, and toggle should show it', async ({ page, isMobile }) => {
  ...
  test('Main navigation should be visible natively on desktop without toggle', async ({ page, isMobile }) => {
    if (isMobile) test.skip();
  ...
  test('Timeline layout should not overflow on mobile', async ({ page, isMobile }) => {
    if (!isMobile) test.skip();
  ...
  test('AI Portfolio section is linked from main site and ordered correctly', async ({ page, isMobile }) => {
    await page.goto('/');

    // Look for the link to the AI Portfolio
    const aiLink = page.locator('nav[aria-label="Main Navigation"] a[href="/ai/"]');
    ...
    const isOrderedCorrectly = await page.evaluate(() => {
      ...
      return !!(aiPortfolio.compareDocumentPosition(contact) & Node.DOCUMENT_POSITION_FOLLOWING);
    });
