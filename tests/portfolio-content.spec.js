...
test.describe('Portfolio Content & Structure', () => {
  ...
  test('contains exactly 12 case studies with correct headers', async ({ page }) => {
    ...
    for (let i = 0; i < expectedHeaders.length; i++) {
      await expect(casesTrack.nth(i).locator('h3')).toContainText(expectedHeaders[i]);
    }
  });

  test('contains exactly 6 open source contributions with correct headers', async ({ page }) => {
    ...
    for (let i = 0; i < expectedHeaders.length; i++) {
      await expect(osTrack.nth(i).locator('h3')).toContainText(expectedHeaders[i]);
    }
  });

  test('timeline contains correct number of experience items', async ({ page }) => {
...
test.describe('AI Page Content', () => {
  test('contains correct AI content structure', async ({ page }) => {
    await page.goto('/ai/');
    await expect(page.locator('h1')).toContainText('Engineering AI-Powered Solutions');
    await expect(page.locator('#ai-portfolio .case-card')).toHaveCount(1);
    await expect(page.locator('#ai-portfolio .case-card h3')).toContainText('AI-Powered Calls Assistant');
  });
});
